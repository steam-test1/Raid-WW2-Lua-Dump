...
function OverlayPresenter:preprocess_sequence(sequence)
	...
	for _, subtitle in ipairs(sequence:subtitles()) do
		...
		for line = 1, batch_count * lines_per_batch, 2 do
			...
			new_sequence:add_subtitle(CoreSubtitleSequence.Subtitle:new(wrapped_string, subtitle:start_time() + batch_duration * batch, batch_duration, subtitle:color(), subtitle:nationality_icon()))
...
function OverlayPresenter:_clear_workspace()
	if CoreCode.alive(self.__ws) then
		Overlay:gui():destroy_workspace(self.__ws)
	end

	self.__ws = managers.gui_data:create_saferect_workspace()

	self.__ws:set_timer(TimerManager:game())
