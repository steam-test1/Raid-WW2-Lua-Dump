...
function ElementAreaTrigger:_client_check_state(unit)
	...
	if table.contains(self._inside, unit) then
		if not inside or not rule_ok then
			table.delete(self._inside, unit)
			managers.network:session():send_to_host("to_server_area_event", 2, self._sync_id, self._id, unit)
		elseif self._values.trigger_on == "while_inside" then
			managers.network:session():send_to_host("to_server_area_event", 3, self._sync_id, self._id, unit)
		end
	elseif inside and rule_ok then
		table.insert(self._inside, unit)
		managers.network:session():send_to_host("to_server_area_event", 1, self._sync_id, self._id, unit)
	end
end
...
function ElementAreaReportTrigger:_clean_destroyed_units()
	...
	while next(self._inside) and i <= #self._inside do
		...
		if alive(unit) and (not unit:character_damage() or not unit:character_damage():dead() or unit:vehicle_driving()) then
			i = i + 1
		else
...
function ElementAreaReportTrigger:_client_check_state(unit)
	...
	if inside then
		...
		elseif self:_has_on_executed_alternative("rule_failed") then
			managers.network:session():send_to_host("to_server_area_event", 4, self._sync_id, self._id, unit)
		end
	end
end
