...
function ElementInstanceInput:init(...)
	...
	if self._values.instance_name then
		managers.world_instance:register_input_element(self._sync_id, self._values.instance_name, self._values.event, self)
	end
end
...
function ElementInstanceInputEvent:on_executed(instigator)
	...
	if self._values.instance then
		local input_elements = managers.world_instance:get_registered_input_elements(self._sync_id, self._values.instance, self._values.event)
	...
	elseif self._values.event_list then
		for _, event_list_data in ipairs(self._values.event_list) do
			local input_elements = managers.world_instance:get_registered_input_elements(self._sync_id, event_list_data.instance, event_list_data.event)
...
function ElementInstanceOutputEvent:init(...)
	...
	if self._values.event_list then
		for _, event_list_data in ipairs(self._values.event_list) do
			managers.world_instance:register_output_event_element(self._sync_id, event_list_data.instance, event_list_data.event, self)
		end
	end
end
...
function ElementInstancePoint:client_on_executed(...)
	self:on_executed(...)
	managers.worldcollection:check_queued_client_mission_executions()
end
