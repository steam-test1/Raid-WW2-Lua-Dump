...
function CoreWorldCameraManager:scale_worldcamera_fov(aspect_ratio)
	...
	local fov_scale = math.min(base_aspect_ratio / aspect_ratio, CoreWorldCameraManager.MAX_FOV_WIDENING_FACTOR)

	for _, worldcamera in pairs(self._world_cameras) do
		for _, key in ipairs(worldcamera._keys) do
...
function CoreWorldCameraManager:load(param, world_id, translation, yaw)
	if not self:_old_load(param) then
		...
		for name, camera_data in pairs(param.worldcameras) do
			...
			self._world_cameras[name]:load(camera_data, translation, yaw)
		end
...
function CoreWorldCameraManager:destroy_world_cameras(world_id)
	...
	for name, camera in pairs(self._world_cameras) do
...
function CoreWorldCameraManager:play_world_camera_sequence(name, sequence, stay_active)
	...
	elseif stay_active and not self._suspended_game_state_name then
		self._suspended_game_state_name = game_state_machine:current_state_name()
	end
...
function CoreWorldCamera:load(values, translation, yaw)
	...
	Application:trace("[CoreWorldCamera:load]  ", translation, yaw)

	for i, position in ipairs(self._positions) do
		self._positions[i] = position:rotate_with(Rotation(yaw, 0, 0)) + translation
	end

	for i, target_position in ipairs(self._target_positions) do
		self._target_positions[i] = target_position:rotate_with(Rotation(yaw, 0, 0)) + translation
	end
