...
function EnvironmentHandler:update(is_first_viewport, viewport, dt)
	...
	self:set_first_viewport(is_first_viewport, false)
	...
	if not material then
		...
		if post_processor then
			...
			if modifier then
				...
				Application:stack_dump_error("[EnvironmentManager] ", post_processor:modifiers())

				for k, v in pairs(post_processor:modifiers()) do
					Application:stack_dump_error("[EnvironmentManager] ", v:name())
				end
...
function EnvironmentHandler:_add_apply_feeder(feeder)
	if feeder.AFFECTED_LIST then
		...
		for _, affected_feeder in ipairs(feeder.AFFECTED_LIST) do
			table.insert(self._apply_feeder_map, self._feeder_map[affected_feeder.DATA_PATH_KEY])
		end
	else
		table.insert(self._apply_feeder_map, feeder)
	end
end
