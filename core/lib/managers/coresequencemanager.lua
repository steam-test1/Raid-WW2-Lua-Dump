...
function SequenceManager:_add_sequences_from_unit_data(unit_data)
	...
	if seq_manager_filename then
		...
		if manager_node then
			self:_add_unit_element(manager_node, unit_name_key, unit_name, seq_manager_filename)
		end
	end
end
...
function SequenceManager:add_queued_sequences_from_unit_data()
	...
	for i = #self._queue_unit_data, 1, -1 do
		local data = self._queue_unit_data[i]

		if DB:has(self.SEQUENCE_FILE_EXTENSION, data.seq_manager_filename) then
			self._sequence_file_map[data.unit_name_key] = data.seq_manager_filename

			local manager_node = self:_serialize_to_script(self.SEQUENCE_FILE_EXTENSION:id(), data.seq_manager_filename)

			if manager_node then
				self:_add_unit_element(manager_node, data.unit_name_key, data.unit_name, data.seq_manager_filename)
			end

			table.remove(self._queue_unit_data, i)
		end
	end
end
	...
	for _, data in ipairs(manager_node) do
		if data._meta == "unit" then
			if self._unit_elements[unit_name_key] then
				Application:error("Unit has possible duplicate <unit/>-elements in the external sequence manager file ", unit_name, seq_manager_filename, unit_name_key)
			else
...
function SequenceManager:preload_package(package_name)
	...
	for _, unit_data in pairs(units) do
		self:_add_sequences_from_unit_data(unit_data)
	end
end
...
function SequenceManager:count()
	...
	for _, _ in pairs(self._unit_elements) do
		i = i + 1
	end
...
function SequenceManager:save_global_save_data(data)
	if next(SequenceEnvironment.g_save_data) then
		...
		state.g_save_data = table.map_copy(SequenceEnvironment.g_save_data)
	...
	for key, value in pairs(state) do
		if type(value) == "table" then
			if value.__is_unit then
				...
				local unit = worlddefinition:get_unit_on_load(original_unit_id, callback(self, self, "_on_load_unit_done_callback", data))
...
function SequenceManager:do_area_damage(damage_type, attack_unit, pos, range, constant_damage, damage, physic_effect, mass, ignore_unit, direct_attack_unit, ignore_mask, get_damage_func, velocity)
	...
	local hit_being_map = {}
	local ray_caller, ignore_unit_key
...
function SequenceEnvironment.within_box(pos, box_pos1, box_pos2)
	return (box_pos1.x <= box_pos2.x and pos.x >= box_pos1.x and pos.x <= box_pos1.x or pos.x >= box_pos2.x and pos.x <= box_pos2.x) and (box_pos1.y <= box_pos2.y and pos.x >= box_pos1.y and pos.x <= box_pos1.y or pos.y >= box_pos2.y and pos.y <= box_pos2.y) and (box_pos1.z <= box_pos2.z and pos.z >= box_pos1.z and pos.z <= box_pos1.z or pos.z >= box_pos2.z and pos.z <= box_pos2.z)
end
...
function UnitElement:run_sequence(name, endurance_type, source_unit, dest_unit, dest_body, dest_normal, position, direction, damage, velocity, params)
	...
	if sequence then
		...
		if Application:production_build() then
			...
			Application:error("Unable to run non-existing sequence \"" .. tostring(name) .. "\" on unit \"" .. self._name:t() .. "\". Available sequences: " .. tostring(supported_values), true, env)
		end
	end
end
...
function EnduranceElement:damage(env)
	...
	if new_damage >= 0 then
		...
		if extension._damage[env.damage_type] >= self._endurance[env.damage_type] then
...
function EnduranceElement:activate(env)
	...
	local skip = next_endurance_element and self._can_skip and extension._damage[env.damage_type] >= next_endurance_element._endurance[env.damage_type]
...
function AreaDamageElement:init(node, unit_element)
	...
	self._falloff_func_map.linear = nil
...
function AreaDamageElement:get_falloff_key_damage(params, unit, body, dir, hit_pos, damage_type, attack_unit, pos, range, constant_damage, damage, velocity, ignore_unit, direct_attack_unit, ignore_mask)
	...
	if key then
		...
		elseif distance >= key:get_variable("range") then
...
function AreaDamageKeyElement:init(node, unit_element)
	...
	self._mass = self:get("mass")
	self._falloff_func_map = {}
	self._falloff_func_map.linear = self.get_linear_damage
end
...
function EffectElement:activate_callback(env)
	...
	elseif DB:has("effect", name) then
		...
		param_map.position = position
		param_map.velocity = self:run_parsed_func(env, self._velocity) or Vector3()
...
function ObjectElement:set_position(env, position, object_list, local_scope)
	...
	func_name = local_scope and "set_local_position" or "set_position"
...
function ObjectElement:set_rotation(env, rotation, object_list, local_scope)
	...
	func_name = local_scope and "set_local_rotation" or "set_rotation"
...
function SetInflictElement:init(node, unit_element)
	...
	self._set_func_list = {}
	self._set_func_list.damage = self:get("damage")
	self._set_func_list.interval = self:get("interval")
	self._set_func_list.instant = self:get("instant")
	self._set_func_list.enabled = self:get("enabled")
	self._set_func_list.fire_object = self:get("fire_object")
	self._set_func_list.fire_height = self:get("fire_height")
	self._set_func_list.velocity = self:get("velocity")
	self._set_func_list.falloff = self:get("falloff")
end
...
function SetWaterElement:activate_callback(env)
	...
	if name then
		...
		if self._enabled then
			enabled = self:run_parsed_func(env, self._enabled)
		else
...
function SoundElement:play(env)
	...
	if cue ~= nil then
		...
		param_map.gain_abs = self:run_parsed_func(env, self._gain_abs)
		param_map.pitch_abs = self:run_parsed_func(env, self._pitch_abs)
		param_map.crossfade = self:run_parsed_func(env, self._crossfade)
		param_map.t = self:run_parsed_func(env, self._t)
...
function SpawnUnitElement:init(node, unit_element)
	...
	self._skip_parent_destroy = self:get("skip_parent_destroy")
	self._params = {}
...
function SpawnUnitElement:activate_callback(env)
	...
	elseif not rotation then
		...
		if MassUnitManager:can_spawn_unit(Idstring(name)) then
			...
			if unit and self._parameters.copy_source_rotation then
				local joints = loadstring("return " .. self._parameters.copy_source_rotation)()

				for _, joint_name in pairs(joints) do
					local source_joint = env.src_unit:get_object(Idstring(joint_name))
					local unit_joint = unit:get_object(Idstring(joint_name))

					if not source_joint then
						Application:error("SpawnUnitElement:activate_callback - sequence parameter dismember joint " .. joint_name .. " not found in source unit. Skipping.")

						break
					end

					if not unit_joint then
						Application:error("SpawnUnitElement:activate_callback - sequence parameter dismember joint " .. joint_name .. " not found in spawned unit. Skipping.")

						break
					end

					unit_joint:set_local_rotation(source_joint:local_rotation())
				end
			end
		...
		if unit then
			if env.src_unit then
				if alive(env.src_unit) then
					local src_unit_data = env.src_unit:unit_data()

					if src_unit_data then
						src_unit_data.dismembered_part = unit
						src_unit_data.dismembered_parts = src_unit_data.dismembered_parts or {}

						table.insert(src_unit_data.dismembered_parts, unit)
					end
				else
					unit:set_slot(0)
			...
			if alive(env.dest_unit) then
				local dst_unit_data = env.dest_unit:unit_data()

				if dst_unit_data then
					dst_unit_data.spawned_units = dst_unit_data.spawned_units or {}

					table.insert(dst_unit_data.spawned_units, unit)
				end
			end
		end
		...
		local skip_parent_destroy = self:run_parsed_func(env, self._skip_parent_destroy)

		if not skip_parent_destroy then
			if not unit then
