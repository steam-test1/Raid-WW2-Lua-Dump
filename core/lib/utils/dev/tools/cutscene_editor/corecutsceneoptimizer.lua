require("core/lib/utils/dev/tools/cutscene_editor/CoreCutsceneExporter")
...
function CoreCutsceneOptimizer:export_to_database(optimized_cutscene_path)
	...
	local new_animation_blobs = merged_animation and self:_write_animation_blobs(merged_animation, part_path_pattern) or {}
...
function CoreCutsceneOptimizer:_write_animation_blobs(full_animation, part_path_pattern)
	...
	while start_time <= duration do
		...
		self:_write_animation_part(part_path, part, compressed_part)
...
function CoreCutsceneOptimizer:_write_animation_part(path, animation, compressed_animation)
