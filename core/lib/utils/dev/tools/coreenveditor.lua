		...
		if kpro == "shadow_processor" then
			...
			for kmod, vmod in pairs(vpro.modifiers) do
				for kpar, vpar in pairs(vmod.params) do
					self:set_data_path("post_effect/" .. kpro .. "/" .. self:_get_post_process_effect_name(kpro) .. "/" .. kmod .. "/" .. kpar, handler, vpar:get_value())
				end
			end
		end
	end
...
function CoreEnvEditor:_get_post_process_effect_name(post_processor)
	...
	elseif post_processor == "shadow_processor" then
		return "shadow_rendering"
	elseif post_processor == "volumetric_light_scatter" then
		return "volumetric_light_scatter"
	elseif post_processor == "bloom_combine_post_processor" then
		return "bloom_DOF_combine"
	elseif post_processor == "lens_flare_post_processor" then
		return "lens_flare_effect"
	elseif post_processor == "SSAO_post_processor" then
		return "SSAO"
	end
...
function CoreEnvEditor:write_sky(file)
	...
	local cg_value = tostring(self._colorgrade_param:get_value())
			...
			if param:name() == "param" and param:parameter("key") and param:parameter("key") ~= "" and param:parameter("value") and param:parameter("value") ~= "" then
				...
				if not self._sky.params[k] then
					if k == "colorgrade" then
						self._colorgrade_param:set_value(param:parameter("value"))
