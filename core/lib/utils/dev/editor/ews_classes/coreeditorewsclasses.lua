...
function ReplaceUnit:init(name, types)
	...
	local units = EWS:ListBox(panel, "replace_unit_units_1", "LB_SINGLE,LB_HSCROLL,LB_NEEDED_SB,LB_SORT")
	...
	for c, names in pairs(category_map) do
		...
		local units = EWS:ListBox(panel, "replace_unit_units_2", "LB_SINGLE,LB_HSCROLL,LB_NEEDED_SB,LB_SORT")
...
function LayerReplaceUnit:init(layer)
	...
	self._units = EWS:ListBox(self._panel, "layer_replace_units", "LB_SINGLE,LB_HSCROLL,LB_NEEDED_SB,LB_SORT")
...
function UnitTransformDialog:init()
	CoreEditorEwsDialog.init(self, nil, "Unit Transform", "", Vector3(761, 67, 0), Vector3(500, 195, 0), managers.editor:format_dialog_styles("DEFAULT_DIALOG_STYLE,RESIZE_BORDER"))
...
function UnitDuality:init(collisions, pos)
	...
	self._panel = EWS:ScrolledWindow(self._dialog, "", "VSCROLL,TAB_TRAVERSAL")

	self._panel:set_scroll_rate(Vector3(0, 20, 0))
	self._panel:set_virtual_size_hints(Vector3(0, 0, 0), Vector3(1, -1, -1))
