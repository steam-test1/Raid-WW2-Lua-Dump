...
require("core/lib/units/editor/mission/CoreRope")
...
require("core/lib/units/editor/mission/CoreWorldSpawnerElement")
...
function CoreEditor:_init_head_lamp()
	...
	self._light:set_falloff_exponent(1e-08)
	self._light:set_linear_attenuation_factor(0.1)
...
function CoreEditor:open()
	...
	managers.environment_controller:set_volumetric_light_scatter_setting("none")
...
function CoreEditor:pickup_tool()
	...
	self:viewport():vp():set_post_processor_effect("World", Idstring("lens_distortion"), Idstring("empty"))
	self:viewport():vp():set_post_processor_effect("World", Idstring("contour_post_processor"), Idstring("empty"))
	self:viewport():vp():set_post_processor_effect("World", Idstring("colorblind_correction_post"), Idstring("empty"))
	self:viewport():vp():set_post_processor_effect("World", Idstring("lens_flare_post_processor"), Idstring("empty"))
	self:viewport():vp():set_post_processor_effect("World", Idstring("lens_flare_apply_post_processor"), Idstring("empty"))
...
function CoreEditor:run_simulation(with_mission)
	if not Global.running_simulation and (not self._lastdir or true) then
		...
		if not save_success then
			Application:error("[CoreEditor:run_simulation] Saving failed, run simulation aborted.")
...
function CoreEditor:reload_units(unit_names, small_compile, skip_replace_units)
	...
	if small_compile == true then
		...
		for _, unit_name in ipairs(unit_names) do
			...
			local material_config_file = unit_data:material_config()
...
function CoreEditor:putdown_tool()
	...
	self:viewport():vp():set_post_processor_effect("World", Idstring("dof_prepare_post_processor"), Idstring("default"))
...
function CoreEditor:_check_unit_ids_outside_continent()
	...
	for name, continent in pairs(self._continents) do
		...
		for id, unit in pairs(continent._unit_ids) do
			...
			if id_diff < 0 or id_diff >= 100000 then
				Application:error("Unit with id:", id, "is outside range. Unit:", unit)
