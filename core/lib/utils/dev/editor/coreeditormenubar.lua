...
function CoreEditor:build_menubar()
	...
	file_menu:append_item("OPEN_RECENT", "Open Recent...\t" .. self:ctrl_menu_binding("open_recent"), "Open recent world file")
	...
	Global.frame:connect("OPEN_RECENT", "EVT_COMMAND_MENU_SELECTED", self._recent_files_callback, 1)
	...
	view_menu:append_item("AO", "Ambient occlusion", "Change visualization to SSAO")
	view_menu:append_separator()
	...
	Global.frame:connect("AO", "EVT_COMMAND_MENU_SELECTED", callback(self, self, "change_visualization"), "ao_visualization")
...
function CoreEditor:on_post_processor_effect(effect)
	...
	local lens_distortion_effect = effects_off and Idstring("empty") or Idstring("lens_distortion")
	local contour_effect = effects_off and Idstring("empty") or Idstring("contour")
	local chromatic_aberration_effect = effects_off and Idstring("empty") or Idstring("chromatic_aberration")
	local cg_effect = effects_off and Idstring("color_off") or Idstring("colorgrade")
	local lens_flare_effect = effects_off and Idstring("empty") or Idstring("lens_flare_effect")
	local lens_flare_apply_effect = effects_off and Idstring("empty") or Idstring("lens_flare_apply_effect")

	self:viewport():vp():set_post_processor_effect("World", Idstring("dof_prepare_post_processor"), Idstring(effect))
