...
function CoreEditorGroups:load(world_holder, offset)
	...
	for name, data in pairs(load_data.groups) do
		if #data.units > 0 then
			local reference = managers.worlddefinition:get_unit_by_id(data.reference)
			local units = {}
...
function CoreEditorGroups:parse_light(unit, node)
	...
	if node:has_parameter("linear_attenuation_factor") then
		light:set_linear_attenuation_factor(tonumber(node:parameter("linear_attenuation_factor")))
	end
end
...
function CoreEditorGroup:save_lights(file, t, unit, data_table)
	...
	for _, light in ipairs(lights) do
		...
		local laf = light:linear_attenuation_factor()

		file:puts(t .. "<light name=\"" .. light:name():s() .. "\" enabled=\"" .. tostring(light:enable()) .. "\" far_range=\"" .. light:far_range() .. "\" color=\"" .. c_s .. "\" angle_start=\"" .. as .. "\" angle_end=\"" .. ae .. "\" multiplier=\"" .. multiplier .. "\" falloff_exponent=\"" .. falloff_exponent .. "\" linear_attenuation_factor=\"" .. laf .. "\"/>")
	end
end
...
function GroupPresetsDialog:init(files, path)
	...
	self._list = EWS:ListBox(self._panel, "group_presets_groups", "LB_SINGLE,LB_HSCROLL,LB_NEEDED_SB,LB_SORT")
