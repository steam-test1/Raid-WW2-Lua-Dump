...
HealthPackPickup.EVENT_IDS.medic_share_health = 1
...
function HealthPackPickup:sync_net_event(event, peer)
	...
	if event == HealthPackPickup.EVENT_IDS.medic_share_health then
		...
		if damage_ext and distance_limit > mvector3.distance(player_pos, source_pos) and not damage_ext:is_downed() and not damage_ext:dead() and not damage_ext:is_perseverating() then
