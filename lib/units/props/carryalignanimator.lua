	...
	if align_object then
		...
		if Network:is_server() then
			...
			if type(carry_unit) == "table" then
				pos, rot = carry_unit.position or Vector3(0, 0, 0), carry_unit.rotation or Rotation(0, 0, 0)
			else
				pos, rot = carry_unit:position(), carry_unit:rotation()
			end
...
function CarryAlignAnimator:_update_loot_animations(t)
	for i = #self._animating_loots, 1, -1 do
		...
		if alive(unit) then
			...
			if lerp_time >= CarryAlignAnimator.SNAP_CAP then
