...
function PlayerStandard:_get_input(t, dt)
	...
	input.btn_comm_wheel_press = pressed and self._controller:get_input_pressed("comm_wheel")
	input.btn_comm_wheel_release = released and self._controller:get_input_released("comm_wheel")
...
function PlayerStandard:_update_crosshair_offset()
	...
	stance = self._state_data.in_steelsight and "steelsight" or self._state_data.ducking and "crouching" or "standing"
...
function PlayerStandard:_stance_entered(unequipped)
	...
	elseif self:_is_carrying_corpse() then
		stances = tweak_data.player.stances.carrying
	else
...
function PlayerStandard:_update_throw_grenade_timers(t, dt, input)
	if self._state_data.throw_grenade_allowed_t and t >= self._state_data.throw_grenade_allowed_t then
...
function PlayerStandard:_update_interaction_timers(t)
	...
	if self._state_data.interaction_delay_t and t >= self._state_data.interaction_delay_t then
		...
		if prime_target.unit_type == unit_type_teammate then
			...
			if rally_skill_data and rally_skill_data.range_sq > mvector3.distance_sq(self._pos, record.m_pos) then
				...
				if needs_revive and rally_skill_data.long_dis_revive then
					voice_type = "revive"
				elseif not needs_revive and rally_skill_data.morale_boost_delay_t and managers.player:player_timer():time() > rally_skill_data.morale_boost_delay_t then
					voice_type = "boost"
					amount = 1
				end
			end
			...
			elseif prime_target.unit:base():char_tweak().is_escort then
				...
				voice_type = prime_target.unit:movement():stance_name() == "cbt" and prime_target.unit:anim_data().stand and "come" or prime_target.unit:anim_data().move and "stop" or prime_target.unit:anim_data().drop and "down_stay" or "down"
...
function PlayerStandard:_start_action_intimidate(t)
	if not self._intimidate_t or t - self._intimidate_t > tweak_data.player.movement_state.interaction_delay then
		...
		local interact_type, sound_name, queue_sound_name
...
function PlayerStandard:_play_distance_interact_redirect(t, variant)
	...
	if not self._state_data.interact_redirect_t or t > self._state_data.interact_redirect_t then
...
function PlayerStandard:_closest_visible_team_member(max_distance_sq, slotmask)
	...
	local cam_fwd = self._ext_camera:forward()
	local my_head_pos = self._ext_movement:m_head_pos()
	local last_error, prime_target_unit
	local cam_fwd = self._ext_camera:forward()
...
function PlayerStandard:_check_action_jump(t, input)
	...
	if action_wanted then
		...
		action_forbidden = action_forbidden or self._gnd_angle and self._gnd_angle > tweak_data.player.max_floor_jump_angle
...
function PlayerStandard:_check_action_mantle(t, input)
	...
	if ray and ray.distance > 0 and max_angle > ray.normal:angle(math.UP) then
		local crouch_pos, close_pos, mid_pos, far_pos
...
function PlayerStandard:_start_action_mantle(t, input, end_pos, is_vault)
	...
	self._state_data.mantle_data.start_pos = self._unit:position()
	self._state_data.mantle_data.end_pos = end_pos
...
function PlayerStandard:_check_action_run(t, input)
	...
	elseif input.btn_run_press or self._running_wanted or input.btn_run_state and self._setting_hold_to_run and not self:ducking() and not self:in_steelsight() and not self:_is_reloading() and not input.btn_primary_attack_state and not self._steelsight_wanted and not self._jump_t then
		if (not self._running or self._end_running_expire_t) and move_dir_length >= tweak_data.player.run_move_dir_treshold then
...
function PlayerStandard:_update_running_timers(t)
	...
	if self._delay_running_expire_t and t >= self._delay_running_expire_t then
...
function PlayerStandard:_check_action_steelsight(t, input)
	...
	if not self._ext_inventory:equipped_selection() or self._ext_inventory:equipped_selection() > PlayerInventory.SLOT_2 then
		return
	end
...
function PlayerStandard:_start_action_reload(t)
	if self._equipped_unit:base():can_reload() then
		...
		if weap_base:clip_empty() then
			if not weap_base:use_shotgun_reload() then
