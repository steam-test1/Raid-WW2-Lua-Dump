...
function MotionpathMarkerUnitElement:clear()
	...
	for i, candidate in ipairs(linked_markers) do
		if not alive(candidate) or candidate:unit_data().unit_id == victim_id then
...
function MotionpathMarkerUnitElement:update_selected(t, dt, selected_unit, all_units)
	...
	if type(selected_unit) == "table" then
...
function MotionpathMarkerUnitElement:bez_interpolate(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, ...)
	...
	bezier3.interpolate(function(s, x, y, z)
		...
		n = n + 1
	end, x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, scale, angle_tolerance, cusp_limit)
...
function MotionpathMarkerUnitElement:bez_draw(id, b, t)
	...
	local n1 = self:bez_interpolate(ax1, ay1, az1, ax2, ay2, az2, ax3, ay3, az3, ax4, ay4, az4, "#ffff00")
