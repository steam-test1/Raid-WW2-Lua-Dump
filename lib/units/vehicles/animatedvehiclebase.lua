...
function AnimatedVehicleBase:update(unit, t, dt)
	...
	if new_pos ~= self._last_pos and alive(self._listener_obj) then
		...
		local veh_speed = new_vel:length()

		veh_speed = math.clamp(veh_speed / 3, 0, 100)
...
function AnimatedVehicleBase:run_module_sequence(module_id, sequence_name)
	...
	if not sequence_name then
		Application:error("No sequence_name param passed\n", self._unit:model_filename(), "\n")

		return
	end

	if self._modules[module_id].unit:damage():has_sequence(sequence_name) then
		self._modules[module_id].unit:damage():run_sequence_simple(sequence_name)
	else
		Application:error(sequence_name, "sequence does not exist in:\n", self._modules[module_id].unit:name())
	end
end
...
function AnimatedVehicleBase:set_local_anim_position_start(x, y, z)
	self._local_anim_position_start = Vector3(tonumber(y), tonumber(x), tonumber(z)) * 100
end

function AnimatedVehicleBase:set_local_anim_position_end(x, y, z)
	self._local_anim_position_end = Vector3(tonumber(y), tonumber(x), tonumber(z)) * 100
end
...
function AnimatedVehicleBase:offset_position_rotation_forward()
	local current_rot = self._unit:rotation()
	local rot = Rotation(current_rot:yaw(), current_rot:pitch(), current_rot:roll())
	local offset_vec = self._local_anim_position_end - self._local_anim_position_start

	mvector3.rotate_with(offset_vec, rot)

	local current_pos = self._unit:position()

	self._unit:set_position(current_pos + offset_vec)
	...
	self._unit:set_rotation(Rotation(current_rot:yaw() + dyaw, current_rot:pitch() + dpitch, current_rot:roll() + droll))
end
...
function AnimatedVehicleBase:offset_position_rotation_backward()
	...
	local droll = self._local_anim_rotation_end[3] - self._local_anim_rotation_start[3]
	local current_rot = self._unit:rotation()
	local rot = Rotation(current_rot:yaw() + dyaw, current_rot:pitch() + dpitch, current_rot:roll() + droll)
	local offset_vec = self._local_anim_position_end - self._local_anim_position_start
