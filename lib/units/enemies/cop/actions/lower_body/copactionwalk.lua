...
function CopActionWalk:_init()
		...
		for i, nav_point in ipairs(self._nav_path) do
			if not nav_point.x then
				...
				nav_point.element.nav_link_wants_align_pos = nav_point.element.nav_link_wants_align_pos or function(element)
					Application:debug("[CopActionWalk:_init()] Appending something that is not nav_link", inspect(self))
	...
	if #self._simplified_path == 2 and not self._NO_RUN_STOP and not self._no_walk and self._haste ~= "walk" and not (mvec3_dis(self._curve_path[2], self._curve_path[1]) < 120) then
	...
	if Network:is_server() then
		...
		end_pose_code = not action_desc.end_pose and 0 or action_desc.end_pose == "stand" and 1 or 2
...
function CopActionWalk:_sanitize()
	if not self._ext_anim.pose then
		Application:error("[CopActionWalk:init] no pose in anim:  ", self._unit, self._machine:segment_state(idstr_base), self._unit)

		local res = self._ext_movement:play_redirect("idle")

		if not self._ext_anim.pose then
			Application:error("[CopActionWalk:init] failed restoring pose with anim", self._machine:segment_state(idstr_base), res)
...
function CopActionWalk:_adjust_move_anim(side, speed)
	...
	if move_side and (side == move_side or self._matching_walk_anims[side][move_side]) then
		...
		if not self._walk_anim_lengths[anim_data.pose] or not self._walk_anim_lengths[anim_data.pose][self._stance.name] or not self._walk_anim_lengths[anim_data.pose][self._stance.name][speed] or not self._walk_anim_lengths[anim_data.pose][self._stance.name][speed][side] then
			Application:error(self._common_data.unit, "Boom...", self._common_data.unit, "pose", anim_data.pose, "stance", self._stance.name, "speed", speed, "side", side, self._machine:segment_state(Idstring("base")))
...
function CopActionWalk:_upd_walk_turn_first_frame(t)
	...
	if type(pos2) ~= "userdata" or pos2.type_name ~= "Vector3" then
		return
	end

	if type(pos3) ~= "userdata" or pos3.type_name ~= "Vector3" then
		return
	end
