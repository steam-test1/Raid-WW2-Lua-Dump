...
function CopLogicAlarm._upd_enemy_detection(data)
	...
	if (not new_reaction or new_reaction < AIAttentionObject.REACT_SHOOT or not new_attention.verified or not (new_attention.dis < CopLogicAlarm.ALARM_SAFE_RANGE)) and (my_data.in_position or not my_data.should_sound_alarm) then
...
function CopLogicAlarm._chk_reaction_to_attention_object(data, attention_data, stationary)
	...
	if not attention_data.verified or attention_data.dis > CopLogicAlarm.ALARM_SAFE_RANGE then
