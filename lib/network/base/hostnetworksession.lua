...
require("lib/network/base/session_states/HostStateLoading")
...
function HostNetworkSession:create_local_peer(load_outfit)
	if Application:editor() then
		...
		managers.blackmarket:_verify_equipped()
	end
...
function HostNetworkSession:chk_drop_in_peer(dropin_peer)
	...
	print("[HostNetworkSession:chk_drop_in_peer]", dropin_peer_id, debug.traceback())
	...
	print("[HostNetworkSession:chk_drop_in_peer] doing drop-in!")
...
function HostNetworkSession:chk_spawn_member_unit(peer, peer_id)
	...
	if not self:chk_all_handshakes_complete() then
		Application:debug("[HostNetworkSession:chk_spawn_member_unit] not self:chk_all_handshakes_complete()")
...
function HostNetworkSession:chk_server_joinable_state()
	...
	if not game_state_machine:current_state():is_joinable() then
		managers.network.matchmake:set_server_joinable(false)
	...
	if not Global.game_settings.drop_in_allowed and not managers.player:local_player_in_camp() then
		managers.network.matchmake:set_server_joinable(false)
...
function HostNetworkSession:do_dropin()
	for peer_id, peer in pairs(self._peers) do
		if self:chk_peer_worlds_loaded(peer) and not peer:spawn_unit_called() then
			Application:debug("[HostNetworkSession:do_dropin()] Trying dropin for", peer_id)
			self:chk_initiate_dropin_pause(peer)
			self:chk_drop_in_peer(peer)
		end
	end
end
