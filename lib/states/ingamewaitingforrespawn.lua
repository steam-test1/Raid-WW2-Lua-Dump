...
function IngameWaitingForRespawnState.request_player_spawn(peer_to_spawn)
	if Network:is_client() then
		...
		local pos_rot = managers.criminals:get_valid_player_respawn_pos_rot()
		...
		if pos_rot then
			...
			managers.network:session():spawn_member_by_id(peer_id, sp_id, false, state_transition)
...
function IngameWaitingForRespawnState:at_enter()
	...
	if managers.buff_effect:is_effect_active(BuffEffectManager.EFFECT_PLAYER_DIED) then
