...
function RaidExperienceManager:_setup()
	...
	if not Global.experience_manager then
		...
		Global.experience_manager.level = Application:digest_value(1, true)
...
function RaidExperienceManager:get_total_xp_for_level(level)
	if not level or level < 1 then
		...
		level = 1
	elseif level > Application:digest_value(RaidExperienceManager.LEVEL_CAP, false) then
...
function RaidExperienceManager:calculate_exp_brakedown(mission_id, operation_id, success)
	...
	if self._loot_redeemed_xp then
		...
		loot_redeemed.id = "xp_additive_loot_redeemed"
		loot_redeemed.amount = self._loot_redeemed_xp
	...
	if not success then
		...
		event_fail_multiplicative.id = "xp_multiplicative_event_fail"
		event_fail_multiplicative.amount = tweak_data:get_value("experience_manager", "level_failed_multiplier") - 1
	...
	if tweak_data.operations.missions[base_id].stealth_bonus and table.contains(self._global.mission_xp, RaidExperienceManager.SCRIPT_XP_EVENT_STEALTH) then
		...
		stealth_multiplicative.id = "xp_multiplicative_stealth"
		stealth_multiplicative.amount = tweak_data.operations.missions[base_id].stealth_bonus - 1
	...
	num_players_multiplicative.id = "xp_multiplicative_num_players"
	num_players_multiplicative.amount = tweak_data:get_value("experience_manager", "human_player_multiplier", managers.criminals:get_num_player_criminals()) - 1

	table.insert(exp_table.multiplicative, num_players_multiplicative)

	local difficulty_multiplicative = {}

	difficulty_multiplicative.id = "xp_multiplicative_difficulty"
	difficulty_multiplicative.amount = tweak_data:get_value("experience_manager", "difficulty_multiplier", tweak_data:difficulty_to_index(game_state_machine._current_state._difficulty or Global.game_settings.difficulty)) - 1

	table.insert(exp_table.multiplicative, difficulty_multiplicative)

	local level_difference_multiplicative = {}

	level_difference_multiplicative.id = "xp_multiplicative_level_difference"
	level_difference_multiplicative.amount = math.lerp(1, tweak_data:get_value("experience_manager", "level_diff_max_multiplier"), self:player_level_difference() / (self:level_cap() - 1)) - 1
...
function RaidExperienceManager:_check_achievements()
	...
	for _, achievement_def in ipairs(achievements) do
		if current_level >= achievement_def.level then
...
function RaidExperienceManager:load(data)
	...
	if not state.version or state.version and state.version ~= RaidExperienceManager.VERSION and self:xp_migration_needed(state.version) then
		...
		if next_level > temp.max_level then
