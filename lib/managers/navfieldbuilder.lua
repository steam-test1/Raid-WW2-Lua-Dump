...
function NavFieldBuilder:load(data)
	...
	if data.quad_borders_x_pos then
		...
		for i_door = 1, #data.door_low_pos do
			...
			mvector3.set_x(door.pos, mvector3.x(door.pos) * quad_grid_size)
			mvector3.set_y(door.pos, mvector3.y(door.pos) * quad_grid_size)
			mvector3.set_x(door.pos1, mvector3.x(door.pos1) * quad_grid_size)
			mvector3.set_y(door.pos1, mvector3.y(door.pos1) * quad_grid_size)
			table.insert(self._room_doors, door)

			local door_dim

			door_dim = door.pos.x == door.pos1.x and "y" or "x"
		...
		for i_vis_group, load_vis_group in pairs(data.visibility_groups) do
			...
			table.insert(self._visibility_groups, visibility_group)

			for _, i_quad in ipairs(load_vis_group.quads) do
				quads[i_quad] = true
				self._rooms[i_quad].vis_group = i_vis_group
			end

			for _, visible_group_id in ipairs(load_vis_group.visible_groups) do
				visible_groups[visible_group_id] = true
			end
		end

		for _, load_seg in pairs(data.segments) do
			local segment = clone(load_seg)

			self._nav_segments[load_seg.id] = segment

			for _, vis_group_id in ipairs(segment.vis_groups) do
...
function NavFieldBuilder:delete_segment(id)
	...
	for i_vis_grp, vis_grp in ipairs(all_vis_groups) do
		for i_quad, _ in pairs(vis_grp.rooms) do
			if all_rooms[i_quad].vis_group ~= i_vis_grp then
				print(" inconsistency1 i_vis_grp", i_vis_grp, "i_quad", i_quad)
			end
		end
	end
	...
	for i_room = #self._rooms, 1, -1 do
		...
		print(" i_room", i_room, "vis_grp", room.vis_group, "segment", room.segment, "seg", room.vis_group and all_vis_groups[room.vis_group].seg)
	...
	for i_vis_grp, vis_grp in ipairs(all_vis_groups) do
		for i_quad, _ in pairs(vis_grp.rooms) do
			if not all_rooms[i_quad] then
				print(" inconsistency2 i_vis_grp", i_vis_grp, "i_quad", i_quad)
			end
		end
	end
end
		...
		for dir_str, exp_dir_data in pairs(expansion_data) do
			for i_segment, segment in pairs(exp_dir_data) do
				...
				if gnd_ray then
					...
					managers.navigation:draw_room(room)
...
function NavFieldBuilder:_trash_room(i_room)
	...
	for i_vis_grp, vis_grp in ipairs(self._visibility_groups) do
		local rooms = {}

		for old_i_room, _ in pairs(vis_grp.rooms) do
...
function NavFieldBuilder:_chk_line_overlap(line1, line2)
	...
	var_1_0 = overlap_min < overlap_max and {
		overlap_min,
		overlap_max,
	}
...
function NavFieldBuilder:_commence_vis_graph_build()
	...
	elseif self._building.stage == 3 then
		...
		self._building.stage = 4
	elseif self._building.stage == 4 then
		self:_update_progress_bar(5, "finalizing")
		self:_cleanup_room_data_1()
		self:_reenable_all_blockers()
...
function NavFieldBuilder:set_nav_seg_metadata(nav_seg_id, param_name, param_value)
	...
	Application:trace("[NavFieldBuilder:_set_nav_seg_metadata]   hoooraaaaaaay")
