...
function GroupAIStateStreet:_begin_assault()
	...
	assault_data.force = math.ceil(self:get_difficulty_dependent_value(tweak_data.group_ai.street.assault.force) * self:_get_balancing_multiplier(tweak_data.group_ai.street.assault.force_balance_mul))
	assault_data.use_smoke = true
	assault_data.use_smoke_timer = 0
	assault_data.force_spawned = 0

	if self._hostage_headcount > 0 then
		assault_data.phase_end_t = assault_data.phase_end_t + self:get_difficulty_dependent_value(tweak_data.group_ai.street.assault.hostage_hesitation_delay)
...
function GroupAIStateStreet:_upd_assault_task(task_data)
	...
	if nr_wanted > 0 and assault_data.phase ~= "fade" and not next(self._spawning_groups) then
		...
		local spawn_group, group_nationality, spawn_group_type = self:_find_spawn_group_near_area(target_area, tweak_data.group_ai.street.assault.groups, nil, nil, verif_clbk)
		...
		if spawn_group then
			...
			self:_spawn_in_group(spawn_group, group_nationality, spawn_group_type, grp_objective, assault_data)
		end
	end
...
function GroupAIStateStreet:_upd_assault_tasks()
	...
	local force_pool = self:get_difficulty_dependent_value(tweak_data.group_ai.street.assault.force_pool) * self:_get_balancing_multiplier(tweak_data.group_ai.street.assault.force_pool_balance_mul)
	...
	elseif assault_data.phase == "build" then
		...
		elseif t > assault_data.phase_end_t or self._drama_data.zone == "high" then
			...
			assault_data.phase_end_t = t + math.lerp(self:get_difficulty_dependent_value(tweak_data.group_ai.street.assault.sustain_duration_min), self:get_difficulty_dependent_value(tweak_data.group_ai.street.assault.sustain_duration_max), math.random()) * self:_get_balancing_multiplier(tweak_data.group_ai.street.assault.sustain_duration_balance_mul)
		end
