...
function VoteManager:host_kick(peer)
	if Network:is_server() and not peer:is_host() then
...
function VoteManager:restart_mission()
	self:_request_vote("restart_mission", self.VOTE_EVENT.request_restart_mission)
end
...
function VoteManager:restart_auto()
	...
	self:_restart_counter("restart")
end

function VoteManager:restart_mission_auto()
	managers.network:session():send_to_peers_except(self._peer_to_exclude, "voting_data", self.VOTE_EVENT.instant_restart_mission, 0, 0)
...
function VoteManager:is_restarting()
	return self._callback_type == "restart" or self._callback_type == "restart_mission"
end
...
function VoteManager:_host_start(vote_type, voter_peer_id, kicked_peer_id)
	...
	elseif vote_type == "restart_mission" then
		managers.chat:feed_system_message(ChatManager.GAME, managers.localization:text("menu_chat_vote_restart_started"))
		managers.network:session():send_to_peers("voting_data", self.VOTE_EVENT.process_restart_mission, 0, 0)
	end
...
function VoteManager:_host_finish(success)
	...
	if success then
		...
		elseif vote_type == "restart" or vote_type == "restart_mission" then
...
function VoteManager:_start(type, kick_peer)
	...
	elseif type == "restart" then
		managers.chat:feed_system_message(ChatManager.GAME, managers.localization:text("menu_chat_vote_restart_started"))
	elseif type == "restart_mission" then
		managers.chat:feed_system_message(ChatManager.GAME, managers.localization:text("menu_chat_vote_restart_started"))
	end
...
function VoteManager:network_package(type, value, result, peer_id)
	if Network:is_server() then
		...
		elseif type == self.VOTE_EVENT.request_restart_mission then
			if self:_host_start("restart_mission", peer_id) then
				self:_host_register(peer_id, self.VOTES.yes)
				self:message_vote()
			end
	...
	elseif type == self.VOTE_EVENT.process_restart then
		self:_start("restart")
	elseif type == self.VOTE_EVENT.process_restart_mission then
		self:_start("restart_mission")
	elseif type == self.VOTE_EVENT.stopped then
		...
		elseif vote_type == "restart" and value == 1 then
			self:_restart_counter("restart")
		elseif vote_type == "restart_mission" and value == 1 then
			self:_restart_counter("restart_mission")
		end
	...
	elseif type == self.VOTE_EVENT.instant_restart then
		self:_restart_counter("restart")
	elseif type == self.VOTE_EVENT.instant_restart_mission then
		self:_restart_counter("restart_mission")
...
function VoteManager:update(t, dt)
	...
	if self._callback_counter then
		...
		if self._callback_type == "restart_mission" then
			self._callback_counter_print = self._callback_counter_print or tweak_data.voting.restart_delay
...
function VoteManager:message_vote()
	...
	elseif self._type == "restart" then
		...
		dialog_data.title = managers.localization:text("dialog_mp_vote_response_title")
		yes_button.text = managers.localization:text("dialog_yes")
		no_button.text = managers.localization:text("dialog_no")
		timeout_choice = self.VOTES.cancel
	elseif self._type == "restart_mission" then
		message = "dialog_mp_vote_restart_mission_message"
