...
function DropLootManager:enemy_drop_item(tweak_table, killer_unit, position, rotation)
	...
	if alive(spawned_pickup) then
		...
		if pickup_type == "health" and managers.buff_effect:is_effect_active(BuffEffectManager.EFFECT_ENEMY_LOOT_DROP_DESPAWN_HEALTH) then
			...
			despawn_time = pickup_type == "ammo" and managers.buff_effect:is_effect_active(BuffEffectManager.EFFECT_ENEMY_LOOT_DROP_DESPAWN_AMMO) and managers.buff_effect:get_effect_value(BuffEffectManager.EFFECT_ENEMY_LOOT_DROP_DESPAWN_AMMO) or despawn_time
		end
