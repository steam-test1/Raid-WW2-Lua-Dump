...
function ObjectInteractionManager:interact(player)
	if alive(self._active_unit) then
		local interacted, timer = self._active_unit:interaction():interact_start(player, self._active_locator)
...
function ObjectInteractionManager:_update_targeted(player_pos, player_unit)
	...
	for i = 1, self._close_freq do
		...
		if alive(unit) then
			if not unit:interaction() then
				Application:throw_exception("[ObjectInteractionManager:_update_targeted] Missing interaction for unit (possible UnitResource problem?): " .. inspect(unit))
			elseif unit:interaction():active() and not self:_in_close_list(unit) and mvec3_dis(player_pos, unit:interaction():interact_position()) <= unit:interaction():interact_distance() then
	...
	if #self._close_units > 0 and not blocked and not driving and not bipod_deployed then
		local active_unit, current_dot, closest_locator
		...
		if self._close_test_index > #self._close_units then
					...
					if current_dot < dot or alive(last_active) and unit == last_active and dot_limit < dot then
						...
						if (not interact_axis or mvector3.dot(mvec1, interact_axis) < 0) and self:_raycheck_ok(unit, camera_pos) then
							...
							if bracket_locator and unit:damage() and unit:damage():has_sequence("show_bracket") then
								unit:damage():run_sequence_simple("show_bracket")
							end
						end
					end
				end
		...
		if active_unit and self._active_unit ~= active_unit then
			...
			self:_hide_bracket_indicator(last_active)
		elseif self._active_locator and self._active_locator ~= last_active_locator then
			...
			self:_hide_bracket_indicator(active_unit)
...
function ObjectInteractionManager:_hide_bracket_indicator(unit)
	if unit and alive(unit) then
		...
		if bracket_locator and unit:damage() and unit:damage():has_sequence("hide_bracket") then
			unit:damage():run_sequence_simple("hide_bracket")
		end
	end
end
